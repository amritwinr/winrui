...
const Finvasia = () => {
    ...
    const brokerData = (arr) => currentBrokerData && currentBrokerData[1]
        ?.filter(item => !["id", "user", "created_at", "updated_at", "quantity", "status"].includes(item)).map((item) => {
            return {
    ...
    const brokerHeading = brokerData(["upper_level", "lower_level"])
    ...
    return (
        <>
            ...
            {showAdd && <TodoHeader onSubmit={onSubmit} id={1}
                                ...
                                <div className='divide-y divide-slate-100 dark:divide-slate-700 -mb-6 h-full'>
                                    <table className='min-w-full divide-y divide-slate-100 table-fixed dark:divide-slate-700'>
                                        ...
                                        <tbody className='bg-white divide-y divide-slate-100 dark:bg-slate-800 dark:divide-slate-700 '>
                                            {currentBrokerData &&
                                                currentBrokerData[0]?.filter(i => i?.type == "Finvasia")?.map((value, index) => (
